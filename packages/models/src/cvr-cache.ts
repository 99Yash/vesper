import { type Redis, TIME_WINDOWS } from "@vesper/lib";
import { type ClientViewMetadata, CVR } from "./cvr";
import { type PullCookie } from "./validators/replicache.validator";

type RedisCVR = {
  notes: Record<string, ClientViewMetadata>[];
...
export class CVRCache {
  ...
  public async getBaseCVR(
    ...
    if (typeof cookie === "object" && cookie !== null && typeof cookie.order === "number") {
      ...
      if (redisCVR) {
        const cvr = new CVR({
          ...
          notes: redisCVR.notes ? CVRCache.convertRedisObjectToMap(redisCVR.notes) : new Map(),
    ...
    const baseCVR =
      previousCVR ??
      new CVR({
        notes: new Map<string, ClientViewMetadata>(),
  ...
  public async setCVR(clientGroupID: string, order: number, cvr: CVR) {
    const redisCVR: RedisCVR = {
      notes: CVRCache.convertMapToRedisObject(cvr.notes),
